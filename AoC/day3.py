...
def main():
    with open("day3.txt") as rucksacks:
        lines = rucksacks.readlines()
    half_rucksacks = list()

    for line in lines:
        new_line = line.strip("\n")
        half_rucksacks.append(new_line[0:(int((len(line) / 2)))])
        half_rucksacks.append(new_line[int((len(line) / 2)):])
    duplicate_items = list()

    for n in range(0,len(half_rucksacks)-1,2):
        item = half_rucksacks[n]
        for char in item:
            if half_rucksacks[n + 1].find(char) != -1:
    ...
    for index, item in enumerate(ALPHABET, start=1):
        if item in duplicate_items:
            priorities += index * duplicate_items.count(item)
...
def part2():
    ...
    duplicate_items = list()
    for n in range(0, len(lines) - 2, 3):
        item = lines[n]
        for char in item:
            if lines[n + 1].find(char) != -1:
                if lines[n+2].find(char) != -1:
                    duplicate_items.append(char)
                    break
